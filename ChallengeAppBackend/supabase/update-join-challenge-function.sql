-- Обновление функции join_challenge для прямой оплаты
-- Убираем проверку баланса, так как оплата происходит до вступления
...
BEGIN
    ...
    -- Пользователь может оплатить и вступить ДО начала челленджа
    ...
    -- УБРАЛИ ПРОВЕРКУ БАЛАНСА - оплата происходит до вступления
    ...
    BEGIN
        ...
        -- УБРАЛИ СПИСАНИЕ С БАЛАНСА - оплата уже прошла
        -- UPDATE public.users
        -- SET balance = balance - v_challenge.entry_fee,
        --     updated_at = NOW()
        -- WHERE id = p_user_id;
        
        -- Create payment record (платеж уже обработан, просто записываем)
